...
**Enforcement grade:** partly mechanical — the gate now registers a TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java, go, typescript): eslint and `tsc` findings are Check A, the `eslint-disable` / `@ts-ignore` / `@ts-expect-error` / `@ts-nocheck` families are Check B, and `tsc --noEmit` is the fail-closed compile precondition. It is a DIFFERENTIAL against the merge base, so it blocks what a change ADDS and never what already stands, so a new eslint finding on a changed file blocks. A consuming repo typically also runs `prettier --check` and `eslint --max-warnings 0` on its own commit path, which is stricter than a merge-base differential but is that repo's wiring, not the gate's. Naming, file organisation, and anything eslint has no rule for stay review.
