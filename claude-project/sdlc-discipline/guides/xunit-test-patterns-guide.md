...
A sound default is: Fresh Fixture (Transient) + Minimal Fixture + Implicit Setup via pytest fixtures + Implicit Teardown via `yield`. Standard Fixture only appears when explicitly justified.
...
When an agent or reviewer is in doubt about *what to do*, they reach for `tdd.md` first. When they need to *name what's wrong*, they reach for `xunit-patterns.md`. The two are not in tension.
