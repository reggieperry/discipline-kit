...
**Enforcement grade:** mechanically enforced — `checkstyle` (google_checks) rides Check A as a source scan, so a new style finding fails the diff, and a new `@SuppressWarnings`, `//CHECKSTYLE:OFF`, or `//NOPMD` is caught by Check B.
