...
**Enforcement grade:** review and convention, and unenforceable by construction — this file governs how the author drives the compiler, not what the code contains. Nothing detects a fourth attempt at the same compile error, an unbounded log read, or a full-suite run used to answer a does-it-compile question. The timings below were measured on a reference repository and are indicative, not a promise about yours.
