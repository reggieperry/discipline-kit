...
**Enforcement grade:** partly mechanical — the gate now registers a TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java, go, typescript): eslint and `tsc` findings are Check A, the `eslint-disable` / `@ts-ignore` / `@ts-expect-error` / `@ts-nocheck` families are Check B, and `tsc --noEmit` is the fail-closed compile precondition. It is a DIFFERENTIAL against the merge base, so it blocks what a change ADDS and never what already stands. `tsc --noEmit` is both the precondition and a Check A scanner keyed by TS error code, so a type error a change introduces blocks and a tree that does not compile cannot pass. What that does not reach is whether the types say anything: `any`, a widening cast, and a non-null assertion all type-check, and catching those needs `@typescript-eslint` rules the consuming repo wires.
