...
A change that legitimately needs to touch the judge is not chain work — it parks for the operator. (A PreToolUse path-deny hook that would block the fenced set *inline* is a possible future hardening; the kit ships no such hook today, so the mechanical guarantee is the driver's post-turn re-derivation above, plus the human PR review in the base tier — do not assume an inline block exists.)
