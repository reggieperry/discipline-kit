...
For example: a `core/broker_*.py` module wraps a third-party client library; a `core/llm.py` wraps an HTTP client such as `httpx`; a `knowledge/store.py` wraps a vector database and a relational store. Each hides its dependency behind a narrow interface.
...
For example: an `AccountState` type that validates domain rules (e.g. per-transaction and aggregate limits) through a single `evaluate_proposal` method; an `ApprovedTransaction` type constructable only through validation; a set of computed-value dataclasses in a domain module. Each makes illegal states unrepresentable.
...
For example: a `domain_parameters.py` module is the canonical home for constants that generalize across the system; a computation module generalizes across all inputs and configurations rather than special-casing each.
...
For example: repository Protocols (each with an `InMemory*` second implementation for tests); a thin wrapper over an external client; and a deferred implementation-language decision (a hot numerical path kept in Python now, with a native rewrite behind the same interface later if needed) — the boundary lives at a computation module's public functions.
...
A representative pipeline system's levels are:

| Level | Implementing files | Owns |
| ----- | ------------------ | ---- |
| Domain primitives | `core/state.py`, `domain_parameters.py`, `computation.py` | Types, math, constants |
| Resource and data abstractions | `core/llm.py`, `core/ib_*.py` (deferred), `knowledge/store.py`, `db/` | External-system access |
| Workers | `agents/scan_stage.py`, `agents/analysis_stage.py`, `agents/policy_stage.py`, `agents/record_stage.py` | One pipeline-stage decision |
| Domain managers (planned) | `ScanManager`, `TradeManager`, `ReviewManager` | Coordination of one phase |
| Pipeline primitives | `core/pipeline.py`, `core/events.py`, `core/stage.py` | Orchestration |
...
The hierarchy rule says: `domain_parameters.py` knows about no other file in the system. `computation.py` knows about `domain_parameters.py` only if it needs the published constants, and about nothing else. Workers know about primitives and resources. Managers know about workers. Pipeline primitives know about the abstractions they coordinate but not about the specific workers.
...
Liskov's special guideline for systems that handle asynchronous events (pages 11–12). A staged pipeline that processes events is exactly such a system, so this section maps her recommendation onto that architecture directly.
...
Three rules from this guideline, each of which the per-language `*-concurrency.md` rules encode mechanically:
...
- Domain invariants the system depends on for correctness. A limit rule is hidden inside `AccountState.evaluate_proposal` *as enforcement*, but it is named explicitly in `domain_parameters.py` and documented in `docs/design.md`. The hiding is one layer; the visibility is another.
...
This is the role a system's design specs (`specs/*.md`) should play. Where specs cover only some of these dimensions, they should be extended to cover the rest. A level whose resources, hierarchy placement, and process assignment are not written down has not been designed; it has been coded.
...
- **The god object.** A single type or module that every other module reads from and writes to. A pipeline's shared mutable `PipelineState` blob is the canonical example; the fix splits it into typed events. Invert the dependency: each stage produces and consumes typed values; nothing is shared mutable.
...
- **The leaky resource.** A level that exposes its underlying resource (returns the HTTP client's `Response`, returns the database row tuple, returns a client library's `Order` object). The fix is to translate at the boundary into a domain type.
