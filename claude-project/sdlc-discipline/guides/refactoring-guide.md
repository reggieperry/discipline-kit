...
A principal-engineer reference for using Fowler's discipline and catalog to safely improve the design of existing code. The thesis is Fowler's: refactoring is a controlled technique for improving the design of existing code without changing its observable behavior. The mechanics are recipes — each refactoring has explicit steps that minimize the chance of breaking the system. The vocabulary names the moves so the team and its agents can communicate precisely about transformation. The spine of this guide is *Refactoring: Improving the Design of Existing Code*, 2nd edition (Fowler, Addison-Wesley, 2018), with citations by chapter and page where the position depends on the primary text. Where Liskov's modularity guide, the DDD guide, or the GOOS guide already cover a point, this guide cites them rather than restating.
...
The leverage is concentrated in places where a system is modifying existing code rather than writing it from scratch:

- **A pipeline-engine refactor** — often the largest refactoring effort in a build plan. Replacing a monolithic `core/pipeline.py` with a typed-event coordinator is a series of refactoring moves: Extract Function, Move Function, Replace Constructor with Factory, Replace Conditional with Polymorphism. Fowler's catalog gives the moves to name and the mechanics to apply.
- **A packaging/extraction pass** — lifting project-agnostic `.claude/` content into a reusable pack involves Move Field (across files), Rename, and Extract Class at the directory level.
- **A common patch-loop scope-creep** — an agent finds type errors during review, makes fixes in production code, and bundles them with an unrelated chore. Fowler's Two Hats discipline says: refactoring and feature work are separate modes; never combine. A rule that names this catches the scope creep before it ships.
- **Mechanical cleanup chores** (e.g. adding `from __future__ import annotations` to many modules) — these are refactorings (mechanical Move-equivalent moves at file scope). Agents execute them well without the vocabulary; with it, the work is shaped explicitly as a series of named moves.
...
This lands directly on three patterns that surface during automated-agent operation:

- **The patch-loop scope-creep.** An agent wears both hats simultaneously: feature ("switch `verify.sh` to fail-on-error") and refactoring ("fix the type errors blocking strict mode"). The result is a single PR mixing the chore with production-code edits in a sensitive file. Two-Hats discipline says: file the type fixes as separate work, ship the `verify.sh` change in isolation.
- **A pipeline refactor approached as a single deliverable.** Treated as one big task, the refactor mixes "extract typed events" (refactoring) with "add checkpointing infrastructure" (feature) with "migrate several stages" (mixed). Two-Hats discipline forces a sequence: refactor the pipeline shape first under one hat, then add checkpointing under the other, then migrate stages under the first hat again.
...
Enforcement: when a refactoring rule fires on a PR, the rule asks "is this PR pure refactoring? Pure feature? Or both?" If both, escalate. If pure refactoring, behavior tests should pass without modification. If pure feature, no existing function signatures should change without explicit reason.
...
The Rule of Three explains why you don't extract abstractions on the first occurrence — even when you suspect a pattern. A decoupling rule is best authored after the second pattern instance, not the first; decomposition templates are best calibrated against accumulated data, not authored speculatively.

Preparatory refactoring is the right framing for build-plan #19 (deployable unit + production-host setup) — before items #1-#13 can ship a paper-trading system, the ground needs to be prepared. The structural work is the precondition for the feature work.

### 1.5 Solid tests as a precondition

Refactoring requires solid, self-checking tests. Without them, there is no way to verify behavior preservation, and the discipline collapses.
...
This composes with the GOOS guide. GOOS's binding-tests discipline (a catalog of invariant tests in `docs/invariants.md`) is the foundation that makes refactoring of core domains possible. A pipeline refactor is safe to attempt because the existing stages have unit-level invariants the refactor must preserve.
...
2. **Test** — run the relevant test suite (at minimum `uv run pytest tests/<related-area> -v`). For changes that cross modules, the full suite.
3. **Commit** — make a local commit with a descriptive message naming the refactoring move. Examples:
   ...
   - `refactor: move StatusColor enum from agents/scan_stage.py to domain/types.py`
...
Fowler's catalog gives the team a shared vocabulary. When an agent says "I'm going to refactor this function," that's vague. When it says "I'm going to apply Extract Function on lines 45–52, then Inline Variable on lines 38–39," that's a precise contract: the reader knows exactly what to expect, the mechanics are documented, and the move is verifiable.

An agent should always name the moves it's making. The audit phase should verify that each named move was actually applied (not a different move with the same name). PR descriptions for refactoring work list the moves applied in sequence.
...
This aligns with common CLAUDE.md guidance: "default to writing no comments. Only add one when the WHY is non-obvious." The smells catalog gives a structured way to recognize when a comment is actually requesting a refactoring.
...
Fowler's advice: **track smells but don't reflexively fix them**. The Rule of Three applies — a smell appearing once might be tolerable; appearing three times in code that's actively being modified is a clear signal. An audit phase should flag smells but not block on them; the operator and the team prioritize.
...
This is not an exhaustive list. Fowler's catalog has more entries; the selection above covers the moves you are most likely to encounter in application and automation code. For any move not listed, consult Fowler chapter 5–12 directly — the page numbers above are 2nd ed for cross-referencing.
...
- **Run tests often.** Fowler says "I run the tests as often as I can — typically every few minutes when I am coding." Run `uv run pytest tests/<area> -v` between each refactoring move.
...
- **Keep tests fast.** Slow tests don't get run between refactoring moves. The test suite should run in seconds, not minutes. This is why integration tests and slow external-service tests belong in separate suites — the unit tests must stay fast.
...
A binding-tests catalog (`docs/invariants.md`) is the foundation. Each binding test is self-checking, fast, and captures a domain invariant. As the system's surface grows, the catalog grows; refactoring is safe wherever the catalog covers.

The compile-test-commit rhythm composes with the GOOS guide's red-green-refactor cycle as follows:
...
### 7.4 vs the Decoupling rule

The decoupling rule (`.claude/rules/decoupling.md`) tells agents how to recognize and avoid project-coupling in `.claude/` infrastructure. Its patterns and refactor mappings are themselves a small Fowler-style catalog: each pattern has a recognize-and-fix recipe.

Together: decoupling is a domain-specific application of Fowler's "Move Field across boundary" + "Replace Project-Specific Reference with Parameter" pattern, narrowed to the agent-content layer. The rule's mechanics align with Fowler's small-steps + tests-passing discipline.
...
- **Refactoring databases.** Fowler has a separate book with Pramod Sadalage on database refactoring. Relational-schema work draws on it directly. The current guide focuses on application-code refactoring.
- **Refactoring across deployment boundaries.** Refactoring within a deployable unit is the discipline this guide covers. Refactoring across services or across deployment boundaries (the Strangler Fig pattern at architecture scale) is its own domain. For a single deployable unit, this guide is enough.
...
- `.claude/rules/decoupling.md` — a small Fowler-style catalog applied to the agent-content layer; demonstrates the same "name the pattern, name the fix, run the tests" structure scoped to project-portability.
