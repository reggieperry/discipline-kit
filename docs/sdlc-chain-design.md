...
**The language axis is now closed: five rule sets, five toolchains.** `claude-project/rules/` ships go, java, python, scala and ts, and `sdlc-gate.py` registers a toolchain for each. TypeScript: eslint and `tsc` for Check A, the `eslint-disable`/`@ts-ignore` families for Check B, skip-marker and assertion-site counts plus istanbul coverage for Check D, `tsc --noEmit` as the precondition. Go: golangci-lint for Check A, `//nolint` — bare and per-named-linter, so a widened directive is visible — for Check B, `t.Skip` counts and assertion sites plus coverprofile coverage for Check D, and `go test -run=^$ ./...` as the precondition, chosen over `go build ./...` because the latter never compiles `_test.go` and would let a broken test source truncate every scan that reads test files. Both were calibrated against real corpora, not fixtures alone — a 31-file TypeScript suite, and a Go repo carrying 13 real `//nolint` directives across both key families. **VERIFIED against `_TOOLCHAINS`, not recalled.** This line previously read *three-of-four on rules and three-of-four on the gate*, which undercounted twice over: the rules were five, and the gate's holes were two. **What is still uneven is DEPTH, not coverage** — `go test -race` is not run, `govulncheck` and `npm audit` are not run, coverage-drop is opt-in everywhere, and no property-test parameter is scanned outside Scala and Java.
