...
- **Falsification condition**—non-negotiable, at least one per consequential Decision: the observable that would show this call was wrong. See "The falsifier names its court" below; this is the section that turns an opinion into a bet.
- **Cross-references**—the ADRs this one supersedes or depends on, the stories that implement it, and the named checks that watch its falsifiers.

## The shape of one Decision

A single Decision, carried through the six-part frame—the id fixed, the alternative named, the falsifier a watched check rather than a sentence:
...
D2. Route inter-service claims through the typed schema, not free prose.
    ...
    Falsification: if the typed boundary shows no lower malformed-claim rate
      ...
      Watched by schema-reject-rate-check on the commit path.
```

The check sits on the commit path, not in the prose. The day it reads no improvement, the Decision is answerable—and a superseding Decision cites the same check so the successor inherits the standard it must beat. (The live pattern in this repo: ADR-0001 cites `scripts/chain-refspec-check.sh`, wired into `check.sh`.)
...
Acceptance gate: deep-reason pass, recorded in [reviews/ADR-NNNN-deep-reason.md](reviews/ADR-NNNN-deep-reason.md) (no reachable refutation).
```

A pass that finds a defect is recorded with its findings; a clean pass is recorded as an absence report and signs nothing. A deep-reason verdict is not a signature—it is an adversary's absence report, bounded as such, and the ADR says so rather than dressing it as approval.
...
An ADR advances status only when a named artifact justifies the move—never on assertion alone. `Proposed → Accepted` needs the deep-reason verdict cited on the Status line. `Accepted → Superseded` needs the successor ADR or Decision that replaces it. Implementation is not a status: it is tracked by the implementing story and by the falsifier's named checks running on the commit path. If the artifact does not exist yet, the status does not move yet; "we decided this" is a sentence, not an artifact.
...
- `story-write`—the implementing story that realizes an accepted Decision.
