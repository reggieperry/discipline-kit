# ADR-0002: The chain merges locally; publication is the operator's act
...
revert backstop is therefore sound only if publication is severed from merging. That severing
is this record's first decision, and the rest follow from it.
...
### D1: The chain merges locally and never publishes

An autonomous merge is `git merge --no-ff` into the local `main` of the working clone.
Publication—any push, to any remote—is the operator's act, always. This is what keeps revert
a real backstop rather than an expired one: a bad autonomous merge is a local event, reverted
locally, and nothing reaches the public remote, CI, or any downstream consumer without an
operator deciding to push. The lab's no-GitHub rule is unaffected because the chain pushes
nowhere, not merely not-to-GitHub.
...
`merge_ok` holds when all of the following hold, evaluated by the sequencer per ADR-0001/D2:
...
names the residue as open, and D1 confines its blast radius—a forged advance, an evaded
park, or a deleted refutation can at worst produce a bad local merge, which revert removes,
and never a publication.

### D6: There is no veto window
...
window costs nothing and buys nothing. An unattended merge is an unattended merge, bounded by
revert. Where a positive acknowledgment is wanted, it already exists and is D1's publication
act: the operator's push.
...
bad autonomous merge on local `main`—or on published `main` after an operator push—must not
reach a consuming repository before revert can act; a tag is minted only by the operator, on a
tree they chose to publish.

## Consequences

- Three chain reads currently name `origin/main`—story satisfaction, the story-graph read,
  and dependent unblocking—and all three move to local `main` under D1: the satisfaction
  witness is the local merge commit and its trailer, the graph reads the local trunk, and
  dependents unblock locally because satisfaction is witnessed there. The design's stated
  justification for reading the remote ("a story does not change the graph until a human
  merges it") is exactly what this record changes, so the wording inherits a correction when
  the sequencer is built. Cross-machine consumers see nothing until the operator pushes—that
  lag is D1's deliberate price, not an oversight.
- Unattended throughput is bounded by the operator's push cadence: dependents of a merged
  story unblock locally, but nothing publishes overnight. That is the deliberate price of
  keeping expiry condition one unmet by construction.
- The CI-green conjunct is gone from the merge, honestly: CI runs on the operator's push, and
  a red CI after publication is operator territory, answered by revert or fix-forward at the
  same authority level that pushed.
...
- Two courts are live from this commit: `scripts/revert-sufficiency-check.sh` (the revert
  ...
  (a chain profile enabling autonomous merge without declaring `push = "never"` fails the
  build; with no profile it prints that there is nothing to guard rather than passing
  silently).
...
- **The chain pushes to the remote after merging**: destroys the revert backstop in the first
  target repository on day one (public remote, CI on push, downstream consumption)—the
  measured refutation of §4.12's "none applies on day one." Rejected as unsound today, not
  merely risky.
...
- **CI-green as a merge conjunct**: not locally re-derivable; requiring it forces a
  pre-publication push, which triggers the exact expiry the conjunct was meant to precede.
...
- **The human merge as the standing backstop** (the struck §2 invariant): what D1 retains of
  it is exactly its irreplaceable half—the human *publication*; the merge itself is
  releasable to the machine under D3's conjuncts because revert covers the local blast
  radius.
...
- **D1**: a chain profile enabling autonomous merge without `push = "never"`—watched today
  by `scripts/merge-posture-check.sh`. The operator-side half (a human pushing a tree they did
  not read) is unmechanizable and reads as such: publication is a judgment act by design.
