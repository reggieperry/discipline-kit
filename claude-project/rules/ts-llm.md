...
**Enforcement grade:** review and convention — the gate registers no TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java), so nothing here is scanned, and no scanner anywhere in the kit reads a model boundary.
