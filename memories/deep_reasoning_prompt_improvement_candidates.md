...
2. **Double-search the decisive absences.** Absence is the most failure-prone claim for an LLM (a narrow pattern misses a real hit). Instruct ≥2 differently-phrased searches per absence claim, both reported, and label absence findings "verify before acting."
...
**FOLDED (kit v1.2.0, the agentic-TDD-loop build §5).** All four are now standing template rules, not candidates: item 1 (quote the exact command + raw output for absence claims) and item 2 (double-search the critical absences, both reported, labeled verify-before-acting) are Discipline rule 8 of `reference/deep-reasoning-agent.md`; item 3 (say-vs-does) is rule 10; item 4 (verdict-first, no preamble) is rule 11. The findings contract, baseline control, and ledger routing land beside them (rules 7, 9, 12) and in the `deep-reason` skill's "The adversary's contract" section. This candidate list is closed.
