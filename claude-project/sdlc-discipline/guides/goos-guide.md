...
A principal-engineer reference for using tests to drive both code correctness and design quality. The thesis is Freeman and Pryce's: tests are not merely a verification step — they are the design oracle. Test pain is design feedback; pain ignored is design that won't survive contact with future requirements. The spine of this guide is *Growing Object-Oriented Software, Guided by Tests* (Freeman & Pryce, Addison-Wesley, 2009), with citations by chapter and page where the position depends on the primary text. Where Liskov's modularity guide and the DDD guide already cover a point, this guide cites them rather than restating.
...
A project's CLAUDE.md and command vocabulary should commit to TDD discipline:
...
- A binding-tests catalog (e.g. `docs/invariants.md`) where each invariant has a test name, public surface, and source line range.

What's missing — and what GOOS supplies — is the *test-pain → design-feedback* pattern as a deliberate, named practice. Commands can enforce TDD shape without naming the reading of test pain as a design signal. Many common decisions (deferring a test-suggestion command, recognizing that a pipeline-engine refactor will involve a "breakthrough" rather than incremental moves) are GOOS-shaped without GOOS-grounded vocabulary. This guide closes that gap.

The leverage is concentrated wherever a system is modifying or building a core pipeline — a pipeline refactor, a cost/audit ledger, a policy-gate sequence, an execution stage with an external service, a vector store. Each is a place where the *test-design* decisions and the *production-design* decisions are not separable.
...
The two loops are not optional. A project's commands should enforce the inner loop (Behaviors paired with Cycles). The outer loop is often less formalized; end-to-end pilot tests are what grow it.
...
- **A pipeline-engine refactor:** acceptance tests should drive the entry point through one input end-to-end (real test database, real external API or a carefully chosen HTTP-layer mock, faked execution service), asserting on the persisted record / approved-outcome list.
- **End-to-end pilots:** the full workflow is exercised against real infrastructure (real worktrees, real version-control operations) rather than mocks.
- **Live integration risks remain until they're tested.** Freeman and Pryce explicitly note (p. 31) that the auction-sniper example is weak because tests run against a fake, not the real auction site. The lesson: tests against a fake external service are not the same as tests against the real (or sandbox) service; the gap is real risk and should be documented.
...
| **Integration** | Does our code work against code we can't change? | A repository against its real datastore; an LLM client against the real provider API |
...
### 3.3 Walking skeletons in practice

A system may have *two* walking-skeleton-shaped subsystems, at different layers — illustrated here with a pipeline and its surrounding automation:

**The core pipeline.** A pipeline refactor is essentially the walking-skeleton exercise: a real entry point that runs one input through the new architecture (StageA → StageB → StageC → record written), with a real test database, a real external-API path (or HTTP-layer mock), and a faked execution service. The acceptance test asserts on the persisted record. Everything else — batch inputs, real external actions, capability gates, the full feature set — is later slices.

**The surrounding automation.** An automation pipeline's walking skeleton is a real change through the real version-control flow, exercising plan → build → test → review → merge end-to-end on a deliberately small task. Complex multi-issue scaling, sensitive-file gating, and concurrent scheduling are all "later slices" in the GOOS sense.
...
In a pipeline refactor specifically, the skeleton work surfaces questions about test-DB management, fake-service shape, external-API test-mode policy, and fixture lifecycle. Better to surface those before writing twenty stages.
...
Concretely, acceptance tests should speak in the domain's vocabulary: "screening passes," "policy permits the action," "limit satisfied," "proposal approved," "outcome recorded." Not "row inserted into `decisions` with `status='approved'`." The latter ties the test to today's storage; the former ties it to the domain.
...
This maps naturally onto build items: each build-plan item gets an acceptance test, written before implementation, lifted into the regression suite when the item ships.
...
This lands hard on aggregate-related tests:
...
Binding tests should all have informative failure messages. Where they don't yet, that's an opportunity for incremental improvement when the tests are touched.
...
In a pipeline refactor: start from the entry point, drive the first event in, follow the path to the first observable output. Build the pieces the flow demands; resist building pieces the flow doesn't yet need.
...
Well-written binding tests follow this discipline. A chore-coverage rule should preserve it: when a chore touches a method named in a test, the chore plan's Behaviors section should describe the *behavior*, not the method.
...
For example, an `AccountState` aggregate holds `Position`s, `Equity`, and running totals. Its public surface should be narrower than the union of those — `evaluate_proposal`, `record_fill`, `current_open_risk`. Not getters for every internal piece.
...
This maps to the layered architecture: a domain aggregate should not know about an HTTP client, a vector database, or an external order type. Test-pain corollary: if a test for the aggregate requires setting up an HTTP client, the design has slipped.

### 5.4 Tell, don't ask (chapter 2, pp. 17–18)

This belongs in CLAUDE.md as a design philosophy. GOOS provides the operationalization:
...
The canonical example is `account.evaluate_proposal(proposal)` over `if account.equity * 0.02 > proposal.risk: ...`. The first tells the account to do its job; the second pulls data out and does the account's job for it.
...
The rule lands directly on stage objects: don't mock a `PolicyStage` itself in a test of its `Manager`. Mock the things the stage collaborates with — the repositories — or use a real stage.
...
2. **Hidden environment dependency.** Reading `os.environ` mid-call. Read once at module load, per the config rule.
3. **Setup-heavy tests.** Shared builders like `_make_stage()` and `_baseline_proposal()` reduce this. As the catalog grows, the test-data-builder pattern (§6) should be extended.
...
This lands directly on cost-ledger and decision-audit work:
...
Pattern: `holdsItemsInTheOrderTheyWereAdded`, not `test1` or `add`. For pytest functions: `test_limit_rule_rejects_oversized_proposal`, not `test_evaluate_proposal_invalid`.
...
Builders are warranted for the core domain types (results, proposals, approved/rejected outcomes, decisions, aggregates). A small `tests/builders.py` module is usually worth it once the pipeline lands.
...
- **Precise assertions.** `assert "2%" in decision.reason`, not `assert decision.reason == "Risk amount of $250 exceeds 2% rule cap of $200 "`. The latter locks the test to formatting.
...
This maps directly to mock discipline in pytest:
...
A healthy adapter layer has this shape:

- `LessonRepository` (your interface) — `VectorLessonRepository` (adapter) — integration tests against the real vector store.
- `LLMClient` (your interface) — a concrete provider client (adapter) — integration tests against the real API (or a careful HTTP-layer mock).
- Execution adapter — `Executor` (your interface) — a concrete `AsyncExecutor` (adapter) — integration tests against the service's sandbox.
...
It's worth re-checking whether names like `AccountStore`, `TransactionJournal`, `RunLog` read better than `*Repository`. The repository *role* is incidental; what the thing *is* in the domain is primary.
...
For relational-database work: integration tests should commit and roll back through a real `Transactor`-shaped abstraction. Don't simulate transaction boundaries with savepoints; exercise the real ones.
...
For relational test fixtures, this is worth considering as the test-DB workflow grows. A setup script that creates an empty DB is a start; a per-test cleanup pattern needs design once pipeline tests start writing real records.

## 8. Concurrency and async (chapters 26, 27)

Many such systems are async-heavy: a single asyncio event loop carries every external connection, every LLM call, every event-bus subscriber, and a ProcessPoolExecutor crosses process boundaries for heavy computation. These are exactly the cases where GOOS chapters 26 and 27 are operationally consequential.
...
This design move applies directly to:
...
Stress-testing is most directly relevant to:

- The pipeline event bus under high event volume (race conditions on subscriber lists).
- An external-service connection pool under concurrent demand (client-id allocation, idempotent retries).
- Pipeline interrupt handling (the external-interrupt feature in build item #1).

The invariant catalog (`docs/invariants.md`) is the right home for concurrency-shaped invariants. Candidate entries: pipeline interrupt safety, idempotent execution under retry, worker processes receiving stale data.
...
For pipeline tests, listening is preferred where possible: a typed event bus naturally supports it (subscribe to a `StageCompleted` event; assert when one arrives). Sampling fits I/O-driven cases (poll for a row in a `decisions` table). For database state assertions, sampling at 100ms intervals with a 5-second timeout is a reasonable starting policy.
...
This is directly relevant to error-handling tests:
...
A representative case is a **scheduled monitor** that recomputes some state on a timer. If the schedule lives inside the monitor, end-to-end tests have to wait real time. Externalize: the monitor accepts a `RecomputeRequested` event; production wires a real scheduler that emits the event on its cadence; tests emit the event directly.

The same applies to a **stale-work reaper**: its stale-heartbeat threshold should be part of its configuration, but the *firing* of the reaper should be event-driven, not internal-timer-driven, so tests can drive it deterministically.
...
- An external-service message handler. Either the message is recognized or it isn't; failure policy is "mark this confirmation as failed, alert the operator, do not let the client-library exception propagate into the pipeline."
...
This is the design grain for a cost ledger and decision audit: each is its own listener on the event bus, not a method called by every stage. When a stage emits a `StageCompleted` event, the cost ledger picks it up and charges the cost; the decision audit picks it up and records the audit row. Failures in either listener don't block the other.
...
7. **No mocking of third-party types.** HTTP-client, vector-store, database, and external-service types are wrapped behind your own domain interfaces; tests mock the interfaces.
...
13. **Diagnostic message is in domain language.** A failure tells the reader what went wrong using the domain's vocabulary, not a generic "value mismatch."
...
- **Acceptance-test framework choice.** GOOS uses WindowLicker, FitNesse, JBehave-style frameworks. Python's nearest equivalents are `behave` (BDD), `pytest-bdd`, or simply pytest with carefully-shaped fixtures. Many projects prefer pytest with high-level fixtures rather than a separate BDD framework; the choice is open until the first acceptance tests are written.
- **CI-server choice.** GOOS assumes a CI server runs the full suite on every commit. A local `verify` script plus a hosted CI (e.g. GitHub Actions) serves the same end. The principle — "fast feedback from a real build" — is what matters; the choice of tool is incidental.
...
- `docs/invariants.md` — the binding-tests catalog; a well-shaped one has entries that already follow GOOS ("test names describe features," "watch the test fail with clear diagnostics," etc.).
