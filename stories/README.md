...
A story earns a file when its "done" is contestable and it outlives one sitting. Write it with `story-write`, score it with `story-tighten` before it turns `ready`, and — when it arrives from outside — take it in with `story-intake` (which scores it and derives its parked claims). A story's acceptance criteria carry the anti-weakening contract verbatim (assertion count not reduced, no new suppressions, no new skipped tests versus the merge-base) and enter the courthouse as the loop's claim-first step (`ledger-preregister`).
