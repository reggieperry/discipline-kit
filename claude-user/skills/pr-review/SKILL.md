...
- **Smells** (Fowler) — the ones a reader can see and a scanner cannot: feature envy, primitive obsession, temporal coupling, a name that lies. Name the smell, propose the move. Do **not** re-read for duplication, unreferenced code, cognitive complexity, or single-implementor abstractions: those are the gate's Checks E–H, mechanical and differential, and its `diff` output lists which ran, and under `not_wired` the ones that did not: not wired for this toolchain, no baseline scan, or skipped by `--no-static`. On Scala 3 the same list names each Check A linter the tree has not set up, as `A.<label>: <reason>`, without changing the exit code; other toolchains do not list a Check A tool that did not run. Read that receipt; re-review only the checks it says did not run.
