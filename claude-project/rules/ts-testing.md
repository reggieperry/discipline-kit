...
**Enforcement grade:** partly mechanical — the gate now registers a TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java, go, typescript): eslint and `tsc` findings are Check A, the `eslint-disable` / `@ts-ignore` / `@ts-expect-error` / `@ts-nocheck` families are Check B, and `tsc --noEmit` is the fail-closed compile precondition. It is a DIFFERENTIAL against the merge base, so it blocks what a change ADDS and never what already stands. Check D reads test files for skip markers — `.skip`, `.only`, `.todo`, `xit`/`xdescribe` — which must not rise, and assertion sites (`expect(`, `assert*(`) which must not fall, so a deleted test and a newly-skipped one both block. The anti-weakening list below is therefore enforced in part rather than described. Two gaps stay open and are named rather than left to be found: coverage-drop is opt-in (`--coverage`), and fast-check's `numRuns` is NOT scanned, so a lowered generator budget passes.
