...
**Enforcement grade:** review and convention in the gate — the gate registers no TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java), so nothing here is scanned. A consuming repo typically gets this from `prettier --check` and `eslint --max-warnings 0` on its own commit path, which is stricter than a merge-base diff but is that repo's wiring, not the gate's.
