...
**Enforcement grade:** review and convention in the gate — the gate registers no TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java), so nothing here is scanned. Where a repo wires the `react`, `react-hooks`, and `jsx-a11y` eslint plugins, rules-of-hooks and a11y violations do block; component decomposition and state placement stay review.
