...
**Enforcement grade:** review and convention — the gate registers no TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java), so nothing here is scanned. The `react-hooks` eslint plugin polices dependency arrays where a repo wires it; abort handling and race-free cleanup are review everywhere.
