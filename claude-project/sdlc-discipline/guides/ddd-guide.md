...
A project's CLAUDE.md should commit to DDD vocabulary: domain events over shared mutable state, bounded contexts, aggregate invariants, repositories per context, stages that return typed output. Most build-plan items in a pipeline system are DDD-shaped decisions: replacing a `PipelineState` god-object with typed events; designing a cost-ledger aggregate and its repository; enforcing domain limits as aggregate invariants on an `AccountState`; deciding whether the pipeline stages are one bounded context or several. Once the vocabulary is in place, the *tradeoff grounding* is what this guide adds — Evans is rigorous about *when patterns don't fit*, and that rigor is hard to derive from summary.
...
Concretely: the design docs (`docs/*.md`) are the model in prose; the typed events and frozen dataclasses are the model in code; the domain vocabulary in CLAUDE.md (the named rules, checks, scores, and ratings a domain expert would recognize) is the ubiquitous language. The job is to keep all three in sync as the system grows.
...
- **Domain.** The subject area the software is about. For a running example in this guide: order management and account risk — an account evaluates proposed transactions against limits, approves or rejects them, and records outcomes.
...
- **Entity** (a.k.a. *reference object*). An object defined by identity that persists across state changes. Example: `Transaction`, `Position`, `Account`.
- **Value object.** An object defined by its attributes, not identity, and treated as immutable. Example: `Price`, `RiskAmount`, `StatusColor`, `Score`.
...
| Layer | Responsibility | Example mapping |
| ----- | -------------- | ------------- |
| User Interface | Show information; interpret input | `dashboard/` (Streamlit) |
| Application | Coordinate tasks; direct domain work; thin, stateless of *business* state | Pipeline managers (`ScanManager`, `TransactionManager`, `ReviewManager`) |
| Domain | The model: concepts, rules, business state | `core/state.py` (typed events), `agents/policy_stage.py`, `computation.py`, `domain_parameters.py` |
...
This is a *don't go there* boundary, not a real choice. A dashboard should read from the database using the same domain types the engine writes; there should be no parallel rule-encoding in the UI layer. If a dashboard widget ever wants to "just check the limit rule itself for speed," that's the SMART UI temptation and is rejected on sight.
...
A typical system uses asyncio, an external-service client, a vector store, and an LLM SDK. Each is a framework with its own opinions. The discipline is to wrap each at the infrastructure boundary and let the domain layer see only domain terms. A client library's `Order` type does not appear in `agents/policy_stage.py` or `core/state.py`; it's translated at the connection boundary into your own types.
...
- `Transaction` — identified by transaction ID; persists from initiation through settlement.
- `Position` — identified by instrument + account + open timestamp.
- `Account` (`AccountState`) — identified by account number.
- `Run` (a pipeline execution) — identified by `run_id`; persists for audit.

**Not entities in this model:**

- A `Price` snapshot — has no continuity beyond the bar it represents. Value object.
- An `EMACrossover` event — same.
- A `StatusColor` for a given record — same.
...
A value object is defined by *what it is*, not *who it is*. Two `RiskAmount` instances with the same value are interchangeable. Value objects are best modeled as `frozen=True` dataclasses by default.

**Whole-value rule.** Don't pass a city, state, and ZIP code as three separate strings; pass an `Address` value object. Likewise: don't pass `entry_price`, `stop_price`, `quantity` as three primitives — pass a `Proposal` value object that knows what its risk is. This is the anti-`primitive obsession` rule expressed positively.
...
- Sharing instances safely is possible only when the value is immutable. `frozen=True` makes this automatic.

**Example value objects:**

- `RiskAmount`, `Price`, `Capital` — money types using `Decimal`.
- `StatusColor` (RED/YELLOW/GREEN), `Direction` (LONG/SHORT) — enums acting as value objects.
- `Proposal`, `ApprovedTransaction`, `RejectedTransaction`, `Decision` — domain events flowing through the pipeline.
- `EMACrossover`, `MACDDivergence` — indicator findings.
- `Score` (several components, 1–10 scale) — composite score; whole-value because the aggregate score is meaningless without its components.
...
- **Domain service.** Embeds business rules. Example: a `PolicyGateSequence` service that runs gates 1–N in order against a proposal and returns an `ApprovedTransaction` or `RejectedTransaction`. The per-transaction and aggregate limit rules and the capability gates are domain logic.
- **Application service.** Coordinates a use case. Example: the application service that, on each pipeline tick, asks the scanner for candidates, hands them to the analyzer, sends approved transactions to the executor.
...
The risk in DDD-curious codebases is that everything becomes a `*Service` and entities become anemic data containers. The discipline: ask whether the operation has a natural object home before reaching for a service. Funds transfer involves two accounts, so it doesn't sit on either alone — service. The limit check, on the other hand, is a property of the account's state at the time of the proposal — `account.evaluate_proposal(...)` is a domain method, not a service.
...
A well-aligned project has domain-shaped package names: `core/`, `agents/`, `computation/`, `knowledge/`, `db/`. The watch is for technical-strata creep — a future `core/services/`, `core/handlers/`, `core/managers/` that fragments concepts.

### 4.5 Aggregates

This is the most consequential pattern Evans introduces and the one such systems lean on most. Read carefully.
...
| -------------- | ------- | ---------- |
| `AccountState` | open `Position`s, current `Equity`, period total | aggregate-limit rule (sum of open-position risks ≤ configured fraction of period-start equity); equity ≥ 0; max_positions = ⌊equity / min_position_size⌋ |
| `Transaction` (settled) | entry/exit `Price`s, `Position`, fills, fees, adjustments | net result derivable from entry and exit; never two open instances for same instrument+account |
| `Run` (pipeline execution) | `Decision` events, costs, status | Idempotent on `run_id`; total cost ≤ run budget |
| `Proposal` (pre-execution) | candidate `Position`, computed `RiskAmount`, score, gate decisions | limit rule (proposal risk ≤ configured fraction of account equity); only constructable through `AccountState.evaluate_proposal` |

Note `Position` appears as a member of `AccountState` *and* as a member of `Transaction`. Same conceptual thing, different aggregate roles at different lifecycle phases. The transactional unit changes because the invariants change: while open, position state is bounded by the account's risk math; once closed, the transaction record is a historical aggregate.

**The per-transaction and aggregate limit rules are aggregate invariants.** This is the design-grade reason `ApprovedTransaction` is constructable only through `AccountState.evaluate_proposal`: the construction *is* the invariant check, by Evans's rule 6. `Decision` is not a free-standing object the caller builds; it is the structured outcome of a guarded factory method on the aggregate root.

**Cross-aggregate references go through roots** (rule 5). A `Transaction` references the `Account` it belongs to by ID; it doesn't hold a pointer into the `Account`'s open-position list. A `Run` references the `Transaction`s it produced by ID. This rule prevents the god-object trap — a shared `PipelineState`'s problem is that every stage holds references into every other stage's mutable state.
...
**Repositories per aggregate root, not per class.** For example:

- `AccountRepository` — find by account ID, save updated state.
- `TransactionRepository` — find by transaction ID, find by date range, save settled transactions.
- `RunRepository` — idempotent insert by `run_id`, find by date.
- `LessonRepository` (knowledge layer) — semantic-similarity search; this is a repository even though backed by a vector store.
- `SimilarCaseRepository` — same reasoning.

There is *no* `PositionRepository` because `Position` is not an aggregate root in this model — it's a member of either `AccountState` (open) or `Transaction` (closed). External code asks the account for its open positions; it doesn't query positions independently.
...
For example: `account_repo.find_or_create(account_id)` is the wrong shape. The system either has an account or it doesn't; the difference matters (an account being created today has zero history; an account being loaded has months of activity). Two methods, two intentions.
...
This means the application service ("execute one transaction") owns the database-transaction boundary; the repository participates but does not commit. The exception is read-only operations, where the question doesn't arise.

**Specification-based queries** (p. 95, expanded in chapter 9): a repository can take a `Specification` value object — a named domain rule — and return matching aggregates. `delinquent_invoices = invoice_repo.satisfying(DelinquentInvoiceSpecification())`. This lands as: `closed = txn_repo.satisfying(LossSpecification(threshold=Decimal("-100")))`. The specification is itself a domain concept; named rules become reusable.
...
The interface names *what*, not *how*. `paint.mixIn(other)` says what's happening; `paint.paint(other)` says nothing. Likewise: `account.evaluate_proposal(proposal)` over `account.check_per_txn_and_aggregate_and_count(...)`. The internal implementation may run several checks; the name says what the caller is asking.
...
This is a strong fit because most domain logic operates on immutable value objects. `risk_amount.scaled(factor)` returns a new `RiskAmount` and changes nothing. `score.with_component(component, value)` returns a new `Score`. Commands cluster around aggregate roots: `account.record_fill(fill)` mutates the account's state; `account.evaluate_proposal(...)` is a pure query that returns a typed event.
...
This overlaps directly with a `docs/invariants.md` (an implemented-only invariants catalog). The catalog's entries are aggregate invariants for `AccountState`; the binding tests are the executable form of those assertions. Evans's chapter 10 *is* the design rationale for the catalog.
...
This guides a pipeline-engine refactor: the temptation is to seek a uniform shape for every stage. Evans's rule says find each stage's natural boundary first, then accept that they may differ in size and shape.
...
The most computation-heavy concepts (composite scoring, numerical transforms, ranking) should live in classes whose dependencies are minimal — ideally just primitives, value objects from the same level, and `Decimal`/`datetime` stdlib. This makes those classes testable in isolation and maximally portable across pipeline stages.

A well-factored `computation.py` follows this — its functions take raw input data (array-shaped or serialized bytes) and return typed value objects. Zero dependencies on stages, repositories, or pipeline state. An invariants catalog extends this discipline to computed results.
...
This lands on monetary types and on specifications:
...
Validation and selection are the live cases:

- `RiskBudgetSpecification(account)` — does this proposal fit the aggregate-limit rule given current open risk? Use case: filtering candidate proposals before LLM analysis.
- `StatusAllowsAction()` — does the current status permit the action? Use case: filtering scanner output.
- `LossSpecification(threshold)` — historical filter for the review stage's "what went wrong" analysis.
...
Strategic design is the half of DDD that summaries underweight. It is where the most consequential decisions live: which models exist, how they relate, where the core lives, how systems integrate.
...
**Bounded contexts (in the running example):**

A strategic question CLAUDE.md should answer: **the development-automation infrastructure and the application content are categorically separate.** They are two bounded contexts in one repository. The automation layer (`.claude/`, `scripts/`, root `conftest.py`) processes the application; it is not part of the application's runtime model. The CLAUDE.md "Scope" section is the *context map* for this separation in narrative form.

Within the application itself, an open question is whether the pipeline stages (e.g. a rotation filter, a quality filter, a ranking stage, a timing stage) constitute one bounded context or several. The decision rule (p. 376):
...
For a sequential set of stages like this, the practical answer is often **one bounded context**. The vocabulary is shared (instrument, rank, quality, score, status), the lifecycle is sequential (each stage's output is the next stage's input), there is no team boundary or independent deployment. Trying to split them into separate contexts would create translation overhead with no compensating clarity.

Where the boundary *does* sit, in addition to automation/application:

- **Knowledge context.** A vector store + structured relational store, accessed through `LessonRepository` and `SimilarCaseRepository`. Internally, a lesson is a different concept from a transaction: it has embeddings, similarity scoring, retrieval-by-vector. The lesson context has its own vocabulary (corpus, similarity, embedding). The integration to the main context is through a clean repository interface — the analysis stage asks for "five most-similar past cases" and gets back typed lessons.
- **Execution context.** External-service connection management is its own context with its own vocabulary (client-id, request-id, contract, bracket order). Domain code does not see those terms; an anticorruption layer translates.
...
A context map is a written and diagrammatic description of which models exist and how they relate. In the running example:

| From | To | Relationship | Notes |
| ---- | -- | ------------ | ----- |
| Application | Automation | Customer/Supplier (application is customer, automation supplies the dev workflow) | The automation's job is to make development reliable; CLAUDE.md scope rules are the contract |
| Application | Knowledge | Repository/Anticorruption | Domain code asks repositories; vector-store/relational specifics hidden |
| Application | External service | Anticorruption Layer | Client-library types translated to domain types at the boundary |
| Application | External data feeds | Anticorruption Layer | Raw feed payloads translated to domain `Holding`, `Record` value objects |
| Application | LLM provider API | Conformist (small interface) | LLM client wraps an HTTP client thinly; the provider's request/response shape is the interface; we don't fight it |
...
**Anticorruption layer** is the most consequential of these here (chapter 14, pp. 365–370):
...
An external-service integration is the textbook ACL case. A client library exposes `Order`, `Contract`, `Execution` types that don't fit your model. The ACL exposes `submit_bracket_order(approved_transaction) -> OrderConfirmation`, internally calling the client library with translated arguments and translating fills back to your types. The catch-and-translate of an HTTP client's `ConnectError` and a database driver's `OperationalError` mentioned in the modularity guide §2.1 is the same pattern.

**Conformist**, on the other hand, is when the upstream's model is fine and integration cost dominates. A typical relationship with an LLM provider's SDK is conformist: its request/response shape is the interface. Building an ACL over a well-designed SDK would be over-engineering — the SDK is already a clean translation of the HTTP API.
...
**Core domain.** The part of the model that is the project's reason to exist. In the running example it is the decision logic — the thing an off-the-shelf tool does not do for you:

- The per-transaction and aggregate risk-limit envelopes.
- The multi-screen filtering and status logic for direction and timing.
- The quality/value composite for selection.
- The composite scoring for systematic entry-quality assessment.
- The policy-gate sequence enforcing all of the above.

These are the things the system does *that a generic framework does not*. They cannot be bought.

**Generic subdomains** (p. 405): cohesive subdomains that are not the project's reason to exist. In the running example:

- External-service connection management. Generic. Could in principle be replaced by a different provider; the domain doesn't care which.
- Cost tracking. Generic. Fee and adjustment capture is the same for any system.
- Vector storage (knowledge context). Generic. One vector store could be swapped for another.
- Dashboard. Generic. One frontend could be swapped for another.
...
In the running example this lands on:

- The pipeline orchestration primitives (`StageCoordinator`, `Gate[T]`, `Retry`, `EventBus`). These are mechanisms; they don't represent the domain. They're a framework that the domain managers (`ScanManager`, `TransactionManager`, `ReviewManager`) use.
- A serialized cross-process computation transport. A mechanism, not a domain concept.
...
The domain model says "compute step A, then step B, then the status." The mechanism is whatever ProcessPoolExecutor + serialized transport + asyncio.gather does to make that fast and reliable.

**Domain vision statement** (p. 422). One page describing the core domain and the value it brings. CLAUDE.md plus the design docs collectively serve this role; a focused one-page statement of "what this system is" is worth distilling once the build plan reaches a mid-project milestone.

### 6.5 Patterns from chapter 16 often not worth adopting

Three patterns from chapter 16 often deserve a deliberate "not here, and here's why":

- **System metaphor.** Evans is skeptical even in the book — "Few projects have found really useful METAPHORS, and people have tried to push the idea into domains where it is counterproductive" (p. 442). When there's no candidate metaphor, skip.
- **Knowledge level.** Powerful but heavy: a configuration model that constrains an operational model. "If the KNOWLEDGE LEVEL becomes complex, the system's behavior becomes hard to understand. The users (or superuser) who configure it will end up needing the skills of a programmer" (p. 469). When there are no end-user-configurable rules and the domain rules are fixed by the methodology, skip.
- **Pluggable component framework.** Heavy; presupposes multiple independent applications sharing a core. "A PLUGGABLE COMPONENT FRAMEWORK should not be the first large-scale structure applied on a project, nor the second" (p. 473). Skip until there's a second application sharing the same core.

**Responsibility layers** (p. 451) is the one large-scale structure pattern that *might* fit a pipeline system eventually:
...
Evans's example layers (Potential / Operations / Decision Support / Policy / Commitment) map roughly onto a staged pipeline: scanning is *Potential*, execution is *Operations*, review is *Decision Support*, the policy-gate sequence is *Policy*. This is suggestive, not prescriptive — Evans warns against imposing a structure before it has emerged (p. 433: "Less is more. … An ill-fitting structure is worse than none"). Revisit once the pipeline refactor settles and the later stages lay down the rest.

## 7. Refactoring discipline (Part III)

Evans's Part III is about *how* the model gets better over time. Three points are non-negotiable.
...
Every domain has an authoritative source — a standard text, a subject-matter expert, a regulatory definition. CLAUDE.md's vocabulary section is the operational form: when speaking about the system, use the domain's exact terms (its named rules, checks, scores, and ratings), never generic synonyms. A concept that doesn't have a domain-vocabulary name probably doesn't exist in the domain.
...
A pipeline refactor is essentially this discipline applied at scale: every rule currently embedded in `core/pipeline.py`'s signal-handling logic becomes a typed event with a named meaning. The limit rule lives in `AccountState.evaluate_proposal`, not in a guard clause inside an analysis stage.
...
- A large pipeline refactor will likely involve a *breakthrough* moment (chapter 8): after several smaller refactorings, the right shape suddenly becomes obvious and a non-incremental change is required. Evans is honest about this (p. 122): "The gospel of refactoring is that you always go in small steps … but to refactor our code to this new model would require changing a lot of supporting code, and there would be few stable stopping points in between." Plan for it; don't pretend it doesn't happen.
- A change you can't explain to a domain expert reading the code is not a deeper model. It's technical virtuosity. If a refactoring removes a vocabulary term the domain uses or introduces a term the domain expert wouldn't recognize, that's a red flag.
...
- **Leaky framework type.** A client library's `Order`, an HTTP client's `Response`, a database driver's `Row` returned to domain code. Fix: anticorruption layer translates at the boundary.
...
1. **Ubiquitous language.** Every public name in the change comes from the domain's vocabulary or is a candidate for it. No generic synonyms (a bland "trend filter" instead of the domain's actual named concept).
...
11. **Bounded context honored.** The change does not silently mix application content and development-automation infrastructure (per the CLAUDE.md scope boundary). Cross-context references go through repositories or explicit translators.
12. **No leaky framework type.** No HTTP-client, external-service-client, database-driver, or vector-store types in the domain layer's public surface.
13. **Documented invariants.** New aggregate invariants are added to `docs/invariants.md`. New binding tests assert them.
14. **Refactoring discipline.** The change either (a) implements an existing build-plan item or (b) makes an implicit concept explicit and the new concept has a domain-vocabulary name.
15. **Domain expert test.** The change can be explained using only the domain's vocabulary, without reference to Python, asyncio, or the storage engine.

A change failing any item is not finished, no matter how green its tests are.

## 11. What this guide does not cover

- **Strategic team organization.** Evans's chapters on team structures, knowledge partitioning, and "six essentials for strategic design decision-making" (chapter 17) presuppose a multi-team project. For a single-developer project, the team-coordination patterns don't apply.
- **The full integration-pattern decision tree.** Evans gives detailed criteria for choosing between OPEN HOST SERVICE and PUBLISHED LANGUAGE, between SHARED KERNEL and CUSTOMER/SUPPLIER, etc. Often only ANTICORRUPTION LAYER and CONFORMIST are live decisions; the rest are noted in §6.3 for completeness but not elaborated.
- **DDD for non-OO languages.** Evans's chapter 5 discusses Prolog and rules engines for domains that resist object-oriented modeling. This guide assumes a solidly OO codebase — Python with frozen dataclasses, `Protocol` types, and class methods.
...
- `docs/invariants.md` — the executable form of Evans's "assertions" (§5.3) for the domain's rules.
- `docs/design.md` — the system's architectural decisions; partially serves the role of Evans's "domain vision statement" (§6.4).
- CLAUDE.md "Design philosophy (apply to all new and refactored code)" — the operating rules that codify several patterns from this guide (Tell Don't Ask, domain events over shared mutable state, repositories per context, stages return typed output).
