...
**Enforcement grade:** partly mechanical — the gate now registers a TypeScript toolchain (`_TOOLCHAINS` holds python, scala, java, go, typescript): eslint and `tsc` findings are Check A, the `eslint-disable` / `@ts-ignore` / `@ts-expect-error` / `@ts-nocheck` families are Check B, and `tsc --noEmit` is the fail-closed compile precondition. It is a DIFFERENTIAL against the merge base, so it blocks what a change ADDS and never what already stands, so where a repo wires `eslint-plugin-security` or `@typescript-eslint`'s injection-adjacent rules their findings now block on introduction. No TypeScript security scanner is wired by default and `npm audit` is not run, so dependency vulnerabilities, secret handling, and the boundary-validation rules below are review.
