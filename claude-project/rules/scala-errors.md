...
- **The `throw` keyword is banned outright.** `DisableSyntax.keywords = [throw]` makes any `throw` a build-failing finding under `sbt check`, even at a hard boundary, so there is no sanctioned `throw` site. Surface an unrecoverable failure as a value (`Either`/`Validated`), or in effectful code raise it through the effect's error channel with `IO.raiseError` over a `NoStackTrace` ADT (`scala-concurrency.md`). Reserve that channel for programmer bugs and unrecoverable invariant breaches, not domain outcomes.
