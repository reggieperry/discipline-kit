...
- **Collect the generators and their `given Arbitrary` instances in one shared test object** in the module that owns the types (an `object Generators` beside the value types), and reuse them downstream through sbt's `test->test` dependency edge (`myLibrary % "compile->compile;test->test"` in the depending project), brought in with a single `import Generators.given`. One canonical generator per type, shared across every module that tests against it, so a corner pinned once is pinned everywhere and two modules cannot drift into subtly different distributions for the same type.
...
- **Pin the hard corners into the generator with `Gen.frequency`**, giving each boundary case its own weighted `Gen.const` branch beside the bulk `Gen.choose`, e.g. `Gen.frequency(1 -> Gen.const(Verdict.Conflict), 1 -> Gen.const(Verdict.Unknown), 3 -> Gen.choose(...))`, so every run produces the empty value, the top, and the contradiction corner rather than reaching them only on a lucky seed. Constructing corner coverage in the generator is what makes the classify/collect check below pass for the right reason instead of being green because the corner was never drawn.
- **Use `classify`/`collect` to confirm the distribution covers the corners**: that the `Verdict` generator actually produces every case (`Unknown`, `Pass`, `Fail`, `Conflict`), not just the common two. A property that never reaches a corner is green for the wrong reason. Keep this as its own committed property whose body is always true, so its only effect is the bucket histogram munit prints: `property("the generator reaches every corner") { forAll { (x: Verdict) => collect(Verdict.corner(x))(true) } }`. It costs nothing on green and turns a silently-narrowed generator (a corner that stops being produced) into a visibly missing bucket. This report property is the guard the anti-weakening rule against generator-narrowing depends on; without it, a narrowed generator just goes quietly green.
...
- **Cap `maxSize` when a law folds or multiplies a generated list of values.** A `combineAll` / `product` over a monoid or semiring lets ScalaCheck's size parameter drive the list length, and a long list can blow the result up combinatorially into an `OutOfMemoryError` on an unlucky seed that reads as a flaky suite. Cap it with `override def scalaCheckTestParameters = super.scalaCheckTestParameters.withMaxSize(8)`. This is safe only when the per-value generators are fixed-size rather than size-driven: the smaller `maxSize` shrinks the list length but not the value space, so per-value corner coverage is unchanged. Cap `maxSize` rather than pinning a lucky seed to hide the blowup.
- **In a multi-module build, pin test execution serial build-wide.** sbt parallelizes test tasks across projects by default; set `Test / parallelExecution := false` and `Global / concurrentRestrictions += Tags.limit(Tags.Test, 1)` so a run is deterministic and memory-bounded, and a heavy law suite in one module cannot run concurrently with another and compound memory pressure. Test determinism is a build-config obligation, not only a per-property one; this lever lives in the build (`scala-modules.md`).
...
- **Gate slow or external suites** (anything hitting a live model, a real store, or the network) so the default `sbt test` stays fast and hermetic, the same split `craft-tdd.md` draws between unit, integration, and end-to-end. The idiom is a conditional `munitIgnore` keyed on an environment flag: `override def munitIgnore: Boolean = !sys.env.contains("RUN_LIVE_STORE")` ignores the suite by default and keeps `sbt check` hermetic, and you run it explicitly with `RUN_LIVE_STORE=1 sbt 'testOnly *SmokeSuite'`. Use a stable `RUN_LIVE_*` naming convention. A permanent env-conditional `munitIgnore` whose default keeps the suite hermetic is a standing fixture, not a newly-disabled test.
...
- A test or property deleted with no equivalent replacement, or a previously-running case newly disabled with `.ignore`, `assume(false, ...)`, `munitIgnore`, a removed registration, or a commented-out body. (The gate scans `munitIgnore` as a skip marker; a permanent env-conditional `munitIgnore` whose default keeps a live suite hermetic, per the Coverage section, is a standing fixture, not a newly-disabled hermetic case.)
