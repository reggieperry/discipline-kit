...
Report a security issue **privately** via a GitHub security advisory on this repository ("Security" → "Report a vulnerability"), **never as a public issue** — a public issue discloses the vulnerability to everyone before it is fixed. Please include a minimal reproduction and the affected files. This repository enables the platform's private vulnerability reporting; an adopter that opens to outside contributions should enable it too (it is the one inbound channel the issue templates deliberately do not cover, because a bug report is a public refutation and a vulnerability must not be).
