...
**Enforcement grade:** review and convention — the Scala scanners the gate runs (scalafix, WartRemover) are purity tools, not security scanners, and Scala has no native `bandit`-equivalent; the honest options are bytecode-side FindSecBugs or `semgrep`, neither yet wired. The gate does not yet police this file's topics mechanically. (This is the hard case of the five: no drop-in scanner exists.)
...
- **Run WartRemover and Scalafix `DisableSyntax`, and fail on a new finding.** The security- and safety-relevant warts (`Null`, `AsInstanceOf`, `IsInstanceOf`, `JavaSerializable`, `JavaConversions`, `OptionPartial`/`TryPartial`/`EitherProjectionPartial`, `Var`, `Throw`) are the static guardrail; `DisableSyntax` bans `null`, `var`, `return`, `while`/`do-while`, `throw`, `asInstanceOf`, `isInstanceOf`, `finalize` overrides, XML literals, and partial `val`-pattern bindings syntactically, so the reader sees the whole referentially-transparent, cast-free, exception-free fragment the build enforces rather than a four-item sample. A differential lint gate blocks new WartRemover/Scalafix findings and new suppressions versus the merge-base; do not add a `@SuppressWarnings` or a `// scalafix:off` to clear a security finding.
- **Compile with `-Wunused:all` and `-Wvalue-discard` (set them in `build.sbt`), and never widen the warning baseline.** A discarded `IO[A]` or a discarded validation result is a dropped check; `-Wvalue-discard` surfaces it. Keep `-Werror` set (Scala 3's spelling for fatal warnings; `-Xfatal-warnings` is retained only as a legacy alias) so a new warning fails the build rather than scrolling past.
