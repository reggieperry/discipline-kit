...
- **The authorship note.** When a claim rests on a report or writeup, that document discloses the true authoring order in one sentence — what was written fresh, what was adapted from an existing template, what a prior session or another author produced — per the disclosure conventions (`ledger/report-conventions.md`). An outcome dressed as sole fresh work is a claim a future reader cannot weigh.
